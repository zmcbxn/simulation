...
- [x] 캐릭터 상세 페이지 (`CharDetailMain.jsx`) - SSE 연동, 요약 패널(이미지/모험단/서버/직업/길드/명성), 새로고침 버튼
  - 모험단명 클릭 시 모험단 검색 이동
  - 새로고침 SSE → refreshed:true면 재조회, false면 쿨다운 alert
- [x] 캐릭터 상세 - 장착장비 탭 (`EquipmentTab.jsx`) 구현
  - 슬롯 고정 순서 (무기/칭호/상의/하의/머리어깨/벨트/신발/목/팔/반/보/마법석/귀)
  - 등급별 색상 배지 + 이미지(넥슨 API) + 강화수치(증폭 시 분홍)
- [x] 검색 홈 - 검색창 포커스 시 최근 검색 이력 드롭다운 (localStorage, 최대 10개, 건별 삭제)
...
### 진행 예정
- [ ] 캐릭터 상세 - 세부스탯/아바타&크리쳐/버프강화/스킬개화/딜표/스킬트리 탭 구현
- [ ] 캐릭터 상세에서 즐겨찾기 추가/제거 버튼
- [ ] 캐릭터 상세 조회 로딩 진행률 (SSE 기반 % 표시)
- [ ] 장비 시뮬레이션 페이지
- [ ] 커스텀 계산기 페이지
- [ ] 랭킹 페이지 구현
- [ ] 경매장 결과 페이지 + 라우트
