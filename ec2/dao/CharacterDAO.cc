...
void CharacterDAO::upsertBaseData(const Json::Value& characterData)
{
    std::string sql = R"(
        ...
        (character_id, server_id, character_name, job_name, job_grow_name, level)
        VALUES ($1, $2, $3, $4, $5, $6)
...
void CharacterDAO::upsertStatusData(const std::string& characterId, const std::string& serverId, const Json::Value& statusData){
    ...
    std::string guildName    = statusData["guildName"].asString();
    int fame = 0;
    ...
    std::string sql = R"(
        ...
        (character_id, server_id, level, job_name, adventure_name, guild_name, fame, raw_data)
...
void CharacterDAO::upsertEquipmentData(const std::string& characterId, const std::string& serverId, const Json::Value& equipmentData){
    ...
    std::string sql = R"(
        ...
        reinforce_value, is_amplified, amplification_name, refine_value, upgrade_info)
